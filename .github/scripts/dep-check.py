...
def format_json_result(
    ...
    """Return structured scan results for the two-workflow pattern."""
    vuln_packages = []
    clean_packages = []
    for pkg in packages:
        key = f"{pkg['name']}:{pkg['newVersion']}:{pkg['ecosystem']}"
        if key in vulns:
            vuln_packages.append({**pkg, "advisories": vulns[key]})
        else:
            clean_packages.append(pkg)
    return {
        "status": "FAIL" if vuln_packages else "PASS",
        "dep_files": dep_files,
        "total_packages": len(packages),
        "vulnerable": vuln_packages,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="markdown",
        help="Output format: markdown (legacy) or json (for workflow_run pattern)",
    )
    ...
    if args.format == "json":
        result = format_json_result(packages, vulns, dep_files)
        _write_output(json.dumps(result, indent=2), args.output)
    else:
